...
class MyPipeline(DiffusionPipeline):
    ...
    @torch.no_grad()
    def __call__(self, audio_path, batch_size: int = 1, num_inference_steps: int = 50):
        # Sample gaussian noise to begin loop
        original_dir = os.getcwd()
        os.chdir('/ImageBind')
        data_dict = { 
            'audio': data.load_and_transform_audio_data([audio_path], self.device), # we only use the text part
        }
        import time
        t0 = time.time()
        with torch.no_grad():
        ...
        aligned_embedding = self.adapter.forward(embeddings)
